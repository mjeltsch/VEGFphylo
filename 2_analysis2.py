...
parser.add_argument("--directory", nargs='?', help = "Specify the subdirectory where the data is!", default = 'user_data/primary_blast_results/')
...
def get_taxon_id_and_phylum(species_name, VERBOSE = True):
    ...
    if species_name in sqlite_species_dict:
        ...
        TAXON_ID = get_taxon_id_from_NCBI(species_name, VERBOSE = VERBOSE)
        ...
        PHYLUM = get_phylum_from_NCBI(TAXON_ID, VERBOSE = VERBOSE)
...
def get_sequence_number_old2(taxon, taxon_data):
    ...
    # given in the header.
    XML_RESULTS_FILE = '{0}/user_data/primary_blast_results/VEGF-C/blast_results_{1}.xml'.format(APPLICATION_PATH, taxon)
...
def write_protein_hitdict_to_file(protein_hitdict):
    ...
    print('\nWriting protein files and doing multiple sequence alignments based on protein_hitdict with {0} unique sequences.'.format(len(protein_hitdict)))
    for key, value in protein_hitdict.items():
        ...
        taxon = value[0]
        print('Excluding proteins. value = {0}'.format(value))
...
# This function takes a string (hit_description) and checks whether any of the strings in the
# two lists (list_of_synonyms, list_of_related_proteins) occurs in the string and returns
# in which of the lists the match was found. If any string in the list starts with 'REGEX:',
# it is treated as a regular expression, and a regex search of the expression is performed
# against the hit_description
def check_relationship(hit_description, list_of_synonyms, list_of_related_proteins):
    # Try synonyms first (more likely to find a category?)
    for synonym in list_of_synonyms:
        if synonym[:6] == 'REGEX:':
            pattern = synonym[6:]
            if re.search(pattern, hit_description) != None:
                print('True positive found: {0}'.format(synonym))
                return 'synonym'                
        else:
            if synonym.lower() in hit_description:
                print('True positive found: {0}'.format(synonym))
                return 'synonym'
    for related_protein in list_of_related_proteins:
        if synonym[:6] == 'REGEX:':
            pattern = synonym[6:]
            if re.search(pattern, hit_description) != None:
                print('Potential false-positive found: {0}'.format(synonym))
                return 'related_protein'
        else:
            if related_protein.lower() in hit_description:
                print('Potential false-positive found: {0}'.format(related_protein))
                return 'related_protein'
    # Returns only 'unknown_protein' if the above two searches do not have any result.
    # The first return statement terminates the function.
    ...
    if args.directory == '':
        ...
        for protein, protein_data in master_dictionary.items():
            ...
            for hit in blastp_result:
                ...
                # BASED ON THE HIT DESCRIPTION (1ST LINE IDENTIFICATION)
                #
                result = check_relationship(str(hit.description).lower(), protein_data[3], protein_data[4])
                if result == 'synonym':
                    ...
                    # HERE WE TRY TO IDENTIFY SYNOMYNS, RELATED PROTEINS & UNKNOWN PROTEINS
                    # BASED ON MAJORITY VOTE FROM BLAST RESULTS (2ND LINE IDENTIFICATION)
...
def run_backcheck_blast(ID, proteindata):
    ...
    try:
        blastp_result = SearchIO.read('user_data/backcheck/{0}.xml'.format(ID), 'blast-xml')
    except Exception as ex:
        ...
        try:
            ...
            while success == False and i < 5:
                ...
                i += 1
                with open('user_data/backcheck/{0}.xml'.format(ID), 'w') as out_handle:
                    out_handle.write(result_handle.read())
                result_handle.close()
                # Read in the data again to check that they contain really blast results
                with open('user_data/backcheck/{0}.xml'.format(ID), 'r') as xml_file:
        ...
        print('Using local backcheck blast results (user_data/backcheck/{0}.xml).'.format(ID))
    blastp_result = SearchIO.read('user_data/backcheck/{0}.xml'.format(ID), 'blast-xml')
...
def get_fully_sequenced_genomes(CSV_FILE, PHYLUM_SPECIES_FILE):
    ...
    # This prints an awful amount of stuff...
    ...
    for line in input_csv_file:
        ...
        print('Iterating through CSV file. Species name: {0}'.format(species_name))
        assembly = line['Assembly']
        print('species_name = {0}'.format(species_name))
        print('Assembly: {0}'.format(assembly))
        # Some species names might need to be sanitized...
        #parts = species_name.split()
        # delete last part if it starts with an upper case character
        #if parts[-1][0].isupper():
        #    del parts[-1]
        # delete last part if it end with a '.'
        #if parts[-1][-1] == '.':
        #    del parts[-1]
        # Recreate species name
        #species_name = ' '.join(parts)
        print('Getting taxon_id and phylum for species {0}'.format(species_name))
        TAXON_ID, phylum = get_taxon_id_and_phylum(species_name, VERBOSE = False)
        ...
        if assembly != 0:
            ...
            print('Adding +1 for {0} ({1} assemblies) to fully sequenced {2}'.format(species_name, assembly , phylum))
        else:
            print('No assemblies for {0}? CSV file shows: {1}'.format(species_name, assembly))
...
def run():
    ...
    for taxon, taxon_data in taxon_dictionary.items():
        ...
        if taxon not in blacklist:
            ...
            try:
                print('Getting new protein data for taxon {0}.'.format(taxon))
